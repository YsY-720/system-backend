...
export class UserService {
    ...
    async initData() {
        for (let i = 0; i < 50; i++) {
            const user = new User();
            user.username = 'YsY_' + i;
            user.password = md5(i + '' + i + '' + i + '' + i + '' + i + '' + i);
            user.email = 'xxx@xx.com';
            user.nickName = 'ysy_' + i;
    ...
    async userLogin(loginUser: LoginUserDto, isAdmin: boolean) {
        ...
        if (user.isFrozen) throw  new HttpException('账号已被冻结', HttpStatus.BAD_REQUEST);
    ...
    //用户冻结
    async freezeUserById(id: number, isFrozen: number) {
